...
import { NotesQuickAdd } from "./NotesQuickAdd";
import {
  KIND_META,
  NOTE_KIND_TAGS,
  RECENCY_LABELS,
...
/**
 * Notes workspace — body component shared by:
 *   - the standalone /notes page (variant="standalone")
 *   - the Notes tab inside the Tasks page (variant="embedded")
 *
 * The only difference between modes is the header treatment. The 3-pane
 * body, capture bar, filtering, and detail pane are identical so muscle
 * memory transfers cleanly between contexts.
 *
 * Variant `embedded` skips the H1 + descriptor — the parent tab already
 * provides them. The search input stays because filtering still needs to
...
export function NotesView({ variant = "standalone" }: { variant?: Variant }) {
  ...
  const personalNotes = useMemo(
    () =>
      notes.filter((n) =>
        (n.tags || []).some((t) =>
          NOTE_KIND_TAGS.includes(t as typeof NOTE_KIND_TAGS[number]),
        ),
      ),
    [notes],
  );
  ...
  // Adjust max-height of list pane based on whether we're embedded under
  // a tab strip. Embedded variant has ~50px extra chrome above to deduct.
  const listMaxH =
    variant === "standalone"
      ? "calc(100vh - 280px)"
      : "calc(100vh - 340px)";
      ...
      {variant === "embedded" && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <input
            ...
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search title, body, tags…"
            className="ml-auto w-44 sm:w-64 bg-bg-secondary/60 border border-border/60 rounded-md px-2.5 py-1.5 text-[12px] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent/50"
          />
        </div>
      )}
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "auto 1fr auto" }}>
        ...
        <section className="min-w-0 relative">
          ...
          {loading && filtered.length === 0 ? (
            ...
            <div
              className="space-y-5 pr-1 overflow-y-auto pb-4"
        ...
        {detailOpen ? (
          <aside className="hidden lg:block w-[440px] flex-none">
            <div
              className="sticky top-4 rounded-2xl border border-border/60 bg-bg-secondary/30 overflow-hidden"
