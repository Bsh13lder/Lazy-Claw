...
- **Upwork MCP Cloudflare resilience** (commit dd2f5aa): `mcp-upwork/.../browser/client.py` adds `_NAV_LOCK` (module-level asyncio.Lock serializing all navigation), `_pick_upwork_page()` that walks every BrowserContext + page and prefers an existing tab already on upwork.com (cookies+warm session pass Cloudflare silently; cold YouTube tab triggers JS-challenge wall even with auth cookies), and `safe_goto()` that wraps `page.goto` with warmup nav to `/nx/find-work/` (when picked tab isn't on Upwork) + a 15-second Cloudflare-pass retry loop. Search migrated from `/nx/find-work/best-matches` to `/nx/search/jobs/` (global board) — was only seeing ~30 personalized recs, now sees the full 100k+ job pool.
