...
Covers:
  ...
  - start_ask_conversation: contact-resolve + autonomous_conversation.start call,
...
def test_matches_instagram():
    assert match_ask_conversation("ask Sam on instagram if he saw my reel")["channel"] == "instagram"


def test_telegram_is_not_a_trigger_channel():
    """Telegram is the CONTROL channel — the Bot API can't read arbitrary
    contact DMs, so it is excluded from VALID_COMMS_CHANNELS. Matching it here
    would create a conversation that could only ever fail at its first send;
    the message must fall through to the brain instead."""
    assert match_ask_conversation("ask Mia via telegram to send the file") is None
...
def _stub_autonomous_conversation() -> types.ModuleType:
    """Return a stub lazyclaw.comms.autonomous_conversation module."""
    mod = types.ModuleType("lazyclaw.comms.autonomous_conversation")
    mod.start = AsyncMock()
    return mod
...
async def test_start_ask_resolves_whatsapp_jid():
    ...
    runner_mod = _stub_autonomous_conversation()
    ...
    with (
        ...
        patch.dict(sys.modules, {
            "lazyclaw.comms.autonomous_conversation": runner_mod,
...
async def test_start_ask_falls_back_to_raw_name_when_contact_not_found():
    ...
    config = object()
    user_id = "u3"
    match = {"who": "Unknown Person", "channel": "instagram", "goal": "when are they free"}
...
async def test_start_ask_returns_error_string_when_runner_unavailable():
    """ImportError on autonomous_conversation → graceful error string, no raise."""
    config = object()
    user_id = "u4"
    ...
    with (
        ...
        patch.dict(sys.modules, {
            "lazyclaw.comms.autonomous_conversation": None,  # triggers ImportError
        }),
    ):
        reply = await start_ask_conversation(config, user_id, match)
