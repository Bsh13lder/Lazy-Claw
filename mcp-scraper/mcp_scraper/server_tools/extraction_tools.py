...
def register_extraction_tools(mcp, get_modules):
    ...
    async def extract_entities(
        ...
        """Extract emails / phones / URLs / dates / IPs / prices from a web page in one call. JS-rendered (Playwright). Returns structured dict per entity type. Use this BEFORE opening a browser when you have a URL and need contact data — it bypasses login walls, costs ~1 LLM call (no follow-up parsing needed), and works on small-business sites where Google snippets miss the email."""
        # Output path validation (Guard A)
        output_error = validate_output_path(output_path, overwrite)
        if output_error:
            return output_error
