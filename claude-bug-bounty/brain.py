...
Supports: Ollama (local), Claude API, claude_cli (LazyClaw $0 path), OpenAI, Grok (xAI)

Provider selection (in order of precedence):
  1. BRAIN_PROVIDER env var  (ollama | claude | claude_cli | openai | grok)
  2. Auto-detect: prefers claude_cli when the `claude` binary + lazyclaw are
     installed (zero per-call cost via Claude Max subscription); otherwise the
     first provider whose API key / server is available.
...
API keys (env vars):
  ...
  (claude_cli needs no key — uses ~/.claude/ subscription auth)
...
class LLMClient:
    ...
    PROVIDER_PRIORITY = ["claude_cli", "ollama", "claude", "openai", "grok"]

    # Default models per provider
    DEFAULT_MODELS = {
        "claude_cli": "sonnet",   # LazyClaw $0 — `claude -p --model sonnet`
    ...
    def _auto_detect(self) -> str:
        # Prefer claude_cli ($0 via Claude Max subscription) when the
        # `claude` binary is on PATH AND the lazyclaw package is importable
        # — that combination means the user has both the auth and the
        # provider wiring ready, and saves them per-token cost.
        if shutil.which("claude") and importlib.util.find_spec("lazyclaw"):
            try:
                self._init_provider("claude_cli")
                if self.available:
                    return "claude_cli"
            except Exception:
                pass
        ...
        rest = [p for p in self.PROVIDER_PRIORITY
                if p not in key_providers and p != "claude_cli"]
    ...
    def _init_provider(self, provider: str) -> None:
        ...
        if provider == "ollama":
            ...
            # LazyClaw $0 path. Routes every request through `claude -p`
            # using the user's Claude Max subscription auth in ~/.claude/.
            # Falls through to other providers if lazyclaw isn't on the
            # PYTHONPATH or the binary is missing.
            ...
            try:
                ...
                self._claw_provider = ClaudeCLIProvider(
                    model=self.DEFAULT_MODELS["claude_cli"],
                )
                self.available   = True
                self.description = "Claude CLI (LazyClaw — $0 via subscription)"
            except Exception:
                pass
    ...
    def chat(self, model: str | None, system: str, user: str,
        ...
        try:
            if self.provider == "ollama":
                ...
                return self._chat_claude_cli(model, system, user, max_tokens, temperature)
    ...
    def _chat_claude_cli(self, model, system, user, max_tokens, temperature) -> str:
        """Route through LazyClaw's ClaudeCLIProvider — $0 via subscription.

        ClaudeCLIProvider.chat() is async, but brain.py exposes a sync
        .chat() interface. We bridge with asyncio.run() — safe here because
        brain.py runs in CLI / sync contexts, never inside an existing event
        loop. If a caller ever hits this from inside a running loop, we
        detect it and use run_until_complete on a fresh loop in a thread.
        """
        import asyncio

        from lazyclaw.llm.providers.claude_cli_provider import LLMMessage

        m = model or self.DEFAULT_MODELS["claude_cli"]
        messages = [
        ...
        async def _run() -> str:
            response = await self._claw_provider.chat(
                messages=messages,
                model=m,
                max_tokens=max_tokens,
