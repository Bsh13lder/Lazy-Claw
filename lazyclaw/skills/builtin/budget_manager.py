...
# ── Compat re-exports ───────────────────────────────────────────────────
# MoveExpenseSkill, ListProjectsSkill, AutoAssignInboxSkill, and
# ListBudgetTopupsSkill moved to budget_inbox.py (2026-07-31, pure move).
# Re-exported here so existing imports (tests import from budget_manager)
# keep working unchanged.
#
# Lazy (PEP 562) rather than a static top-level import: budget_inbox.py
# imports `_fmt_money` back from THIS module at its own top level (shared
# helper), so a static `from budget_inbox import ...` here would deadlock
# with an ImportError whenever something imports budget_inbox.py directly
# before budget_manager.py has been imported. Resolving names lazily on
# first attribute access keeps both import orders safe.
_INBOX_COMPAT_NAMES = frozenset({
    "MoveExpenseSkill", "AutoAssignInboxSkill", "ListProjectsSkill", "ListBudgetTopupsSkill",
})


def __getattr__(name: str):  # noqa: F401 — compat re-export (lazy, see above)
    if name in _INBOX_COMPAT_NAMES:
        from lazyclaw.skills.builtin import budget_inbox
        return getattr(budget_inbox, name)
