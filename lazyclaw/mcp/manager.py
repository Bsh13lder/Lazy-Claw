...
BUNDLED_MCPS = {
    ...
    # Admin-panel-to-API bridge. The browser specialist logs in once and
    # records a panel's real backend endpoints into an encrypted per-site
    # manifest; thereafter `panel_call` replays them as fast direct HTTP calls
    # (reusing the user's live browser session for auth) instead of clicking
    # through the UI. inject_user_context provides the shared Brave profile +
    # CDP port it reads cookies from.
    "mcp-apihunter": {
        "module": "mcp_apihunter",
        "description": (
            "Turn a web admin panel into durable API tools — record its "
            "endpoints once, then post blogs / read analytics / update stock "
            "via direct authenticated API calls (panel_call) instead of "
    ...
    # Google MCP Toolbox for Databases (Apache-2.0, a single Go binary — no
    # Python dep, license-gate safe). Runs PRE-DEFINED, read-only queries
    # against a business database (Postgres/MySQL/SQL Server/BigQuery/SQLite)
    # for monitoring & alerting. Per-user `tools.yaml` is generated by
    # `lazyclaw/mcp/db_toolbox.py` and injected via `per_user_tools_file` (the
    # `bin` branch appends `--tools-file <per-user path>`). Inert until the user
    # sets LAZYCLAW_DB_TOOLBOX_ENABLE=1 AND the `toolbox` binary is on PATH
    # (installed in the Dockerfile). Writes to a business system go through the
    # app / apihunter `panel_call`, never raw SQL.
    # NOTE: verify the `--stdio` / `--tools-file` flags against the pinned
    # toolbox release when bumping it.
    "db-toolbox": {
        "bin": "toolbox",
        "extra_args": ["--stdio"],
        "per_user_tools_file": True,
        "description": (
            "Read business databases (Postgres, MySQL, SQL Server, BigQuery, "
            "SQLite) — run pre-defined, read-only queries for monitoring & "
            "alerts (e.g. low-stock checks). Read-only by design."
        ),
        "optional": True,
        "persistent": True,
        "env_required": ["LAZYCLAW_DB_TOOLBOX_ENABLE"],
    },
    "n8n": {
        "module": "n8n_mcp",
    ...
    for name, info in BUNDLED_MCPS.items():
        ...
        if "module" in info:
            ...
            if not bin_path:
                ...
                continue
            transport = "stdio"
            bin_args = list(info.get("extra_args", []))
            # Per-user tools.yaml: each user's registration row gets its own
            # --tools-file path so a bundled config-driven binary (db-toolbox)
            # reads that user's DB config. A default (empty) file is written so
            # the binary can start before any DB is configured.
            if info.get("per_user_tools_file"):
                from lazyclaw.browser.profile_resolver import resolve_profile_dir
                from lazyclaw.mcp.db_toolbox import ensure_tools_file
                tools_file = ensure_tools_file(resolve_profile_dir(config, user_id))
                bin_args += ["--tools-file", str(tools_file)]
            server_config = {
                "command": bin_path,
                "args": bin_args,
            }
