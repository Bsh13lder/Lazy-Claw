...
- **Hyperlinks ("text with a link")**: `docs/snapshot.py` carries Univer `customRanges` — link text bracketed in `dataStream` by sentinels `CUSTOM_RANGE_START`()/`CUSTOM_RANGE_END`(), `customRanges:[{startIndex,endIndex,rangeId,rangeType:0,properties:{url}}]` (verified vs `@univerjs/core@0.24`). Helpers `build_body_with_runs`/`get_paragraph_runs`/`append_paragraph_with_runs`/`runs_from_markdown`/`make_link_runs`; `get_paragraphs` strips sentinels. `docx_io.py` round-trips real `w:hyperlink`. `append_to_doc` skill takes `link_text`/`link_url` + `[md](url)`. Docs editor mounts `UniverDocsHyperLinkPreset` so links render.
- **Optimistic-concurrency saves (2026-06-12)**: `PUT /api/{sheets,docs}/{id}` accepts optional `base_updated_at`; on mismatch → **409** with `{detail:"conflict", current:{...full decrypted snapshot + updated_at}}` so clients re-base without a second fetch. `name` is now optional on save (omitted → stored name kept; killed mobile's stale-rename clobber). A caller-passed id owned by another user raises `LookupError` → 404 (never a PK `IntegrityError`). Stores raise `SheetConflictError`/`DocConflictError`; requests WITHOUT `base_updated_at` keep last-write-wins (agent skills unaffected). Mobile shows a Reload/Keep-mine banner + refetches on focus; web shows a reload-or-overwrite confirm.
- **Sheet hyperlinks + link converter (2026-06-12)**: Univer sheet links live in the workbook `resources` array under `SHEET_HYPER_LINK_PLUGIN` (`data` is a JSON string → `{subUnitId:[{id,row,column,payload}]}`). `sheets/snapshot.py` helpers `get_sheet_links`/`set_cell_link`/`remove_cell_link`/`convert_urls_to_links` (bare-URL + exact `[text](url)` markdown, trailing-punct trim, idempotent). `POST /api/sheets/{id}/links/convert` + `convert_sheet_links` skill; `set_cells` auto-detects markdown links. Mobile mirrors this in `univer_links.dart`.
- **Document tags (2026-06-12)**: plaintext `tags` JSON column on `sheets`/`docs`/`pdf_files` (same index-sensitivity class as `name`; cleaned to ≤32 tags / ≤40 chars). Sheets/docs set tags via `save(...)` (rides the payload PUT); PDFs via `PATCH /api/pdf/{id}`. Chips + AND-filter + editor on web (`Sheets/Docs/Pdf.tsx`) and mobile (`documents_list_view.dart` + `tag_editor_sheet.dart`).
- **Web full free-preset surface (2026-06-12)**: `Sheets.tsx` mounts the Apache-2.0 preset family — hyper-link, filter, sort, conditional-formatting, data-validation, find-replace, note. NO Univer Pro (charts/pivot/collab/print) — commercial, banned by the MIT-license discipline. All land in the lazy `univer-*` chunk (PWA-precache-excluded).
- **Mobile native grid power-editing (2026-06-12)**: the Flutter sheet editor (`sheet_editor_screen.dart` + `sheet_grid`/`sheet_toolbar`/`sheet_selection`/`sheet_formula_bar`/`sheet_conflict_banner`) gains range selection (drag handle inside the InteractiveViewer child; cached gesture-recognizer factories), a formatting toolbar (B/I/U/S, text/fill color, align, wrap, number formats), row/col insert/delete/clear/width, TSV copy/paste, sort, freeze (visual-only — true pinning is the freeze-v2 TODO), and 50-step undo/redo. Pure model in `univer_model.dart` (`SelRange`/`CellStyleView`/`resolveStyle`/`applyStyle` w/ style-registry dedupe + `formatNumber`, Excel serial epoch 1899-12-30) + `univer_ops.dart` (structure/sort/freeze/TSV); readers use the zero-copy `UniverSheet.rawWorkbook`, mutators copy-on-write.
