...
Standalone MCP servers (7 active + 4 disabled): Active: `mcp-taskai/` (task intelligence), `mcp-lazydoctor/` (self-healing), `mcp-whatsapp/` (WhatsApp via WA-JS), `mcp-instagram/` (Instagram DMs/feed/stories), `mcp-email/` (Gmail/Outlook/IMAP), `mcp-scraper/` (crawl4ai-backed crawl/extract/search bundle), `mcp-upwork/` (Apache-2.0 fork of vanooo/upwork-mcp — 18 tools via CDP, shares Brave profile + cookies). Disabled (source rebuild needed): `mcp-freeride/`, `mcp-healthcheck/`, `mcp-apihunter/`, `mcp-vaultwhisper/`. **`mcp-jobspy` retired 2026-05-14**: SearchJobsSkill now goes through mcp-upwork only (Indeed/Glassdoor/LinkedIn dropped — never produced freelance contracts worth applying to).
...
- **Background tasks**: `run_background` skill → TaskRunner spawns independent Agent → Telegram push on completion. Web UI / WS agents receive the live `TaskRunner` + `TeamLead` via `set_agent_deps()` in `gateway/app.py` (2026-05-14 fix — without it chat_ws Agents had `_task_runner=None` and both `run_background` and `dispatch_subagents` returned "not configured").
- **Delegate tool**: Agent calls `delegate(specialist, instruction)` inline — no separate team lead LLM call.
- **ECO routing**: 4 modes, 3 roles (Brain/Worker/Fallback). HYBRID (default): Sonnet brain + `gemma4:e2b` local worker via Ollama ($0) + Haiku fallback. FULL: Sonnet brain + Haiku workers + Sonnet fallback. CLAUDE: every role through subscription ($0 via `claude` binary), per-role configurable, `claude_transport` sub-mode (`sdk` default | `cli`). MINIMAX: M2.7 brain+worker (Token Plan) + Haiku fallback. `eco_router.py` routes by role. Models from `MODE_MODELS` dict in `model_registry.py`.
...
- **MODE_CLAUDE transport split** (2026-05-14): `claude_transport` = `sdk` (default, official `claude-agent-sdk`, native `tool_use`, `@tool`+`create_sdk_mcp_server`, `strict_mcp_config=True`, `total_cost_usd` reporting) | `cli` (legacy `claude -p`, `SDKUnavailable`-only auto-fallback). Both share brain/worker/fallback model selects. Strict-sticky in `chat` + `stream_chat` + `daily_log` so no path leaks to the paid Anthropic API. Toggle: Web UI Settings or Telegram `/mode claude sdk|cli`. Anthropic splits subscription-funded programmatic use into a separate capped credit pool on **2026-06-15** ($20/$100/$200 by tier) — applies equally to SDK and CLI.
...
- **Claude SDK tool dedup** (2026-05-14): `_dedup_tool_calls` filters duplicate `(name, JSON-args)` blocks within one assistant turn. Sonnet under load occasionally emits 13× of the same call; the CLI provider's `--json-schema` suppressed this implicitly. Logged at WARNING with most-duplicated tool name.
...
- **Upwork MCP message guards** (2026-05-12): `mcp-upwork/.../tools/messages.py:send_message` hard-blocks BEFORE any browser nav on (a) external URLs (`http://`, `www.`, `<domain>.<tld>` for ~30 TLDs) and (b) `\blazyclaw\b` brand-name mentions. Returns `{status: "blocked", offending_token}` so callers rephrase. Upwork's chat filter deletes/strips messages with URLs bilaterally (sender AND recipient see the violation) — guard short-circuits to avoid wasted sends + flagged accounts. Memory: `feedback_upwork_dm_no_links.md` + `feedback_upwork_no_lazyclaw_product_pitch.md`. Plus `connects_to_send` + `milestone_due_date` + `milestone_description` + `project_duration` now reach the MCP signature (were silently dropped before — every submit used Upwork's auto-suggested defaults).
- **Upwork 2026 bubble extractor** (commit d123d93): `get_conversation_messages` parses `[data-test="story-container"]` with header (sender + timestamp via regex) + body split. Carry-forward of `last_sender` / `last_timestamp` so consecutive bubbles from the same author (Upwork omits the header on continuations) still emit with author info. `me_name` param triggers tolerant first-name match for `is_mine` (`"Vato T."` matches `"Vato Tchipa"`). `contact_name` ALWAYS derived from first non-mine sender — page h2 fallback kept capturing UI labels ("Schedule a meeting", "Conversation info"). `_NAV_LABEL_NOISE` extended with conversation-page CTAs.
- **Upwork MCP Cloudflare resilience** (commit dd2f5aa): `_NAV_LOCK` (serialized navigation) + `_pick_upwork_page()` (prefer an existing on-upwork.com tab, cookies pass Cloudflare silently) + `safe_goto()` with 15s Cloudflare-pass retry. Search migrated `/nx/find-work/best-matches` → `/nx/search/jobs/` (global board, 100k+ jobs vs ~30 recs).
- **Drafter pipeline architecture** (Phase 21, in flight): freelancer-survival logic lives in mcp-upwork as agent-agnostic primitives — Phase 1 ✅ (bubble extractor). Phase 2 (`upwork_get_reply_context` returning structured reply-ready prompt + `upwork_validate_message` exposing URL/pitch guards as callable). Phase 3-5: lazyclaw orchestrator side (voice-samples-in-lazybrain + `draft_upwork_reply` skill + Send/Edit/Skip Telegram approval via `escalate_to_human` + cron template + NL rule editor). Any MCP-aware agent (Claude Desktop, lazyclaw, future agents) inherits the drafter behavior — the MCP doesn't call an LLM itself, it returns structured context the caller's LLM consumes.
- **Inbox watcher pivot** (2026-05-13, 6201437): Upwork inbox uses existing `watch_site` / `browser/watcher.py` + heartbeat `_check_watchers` — zero LLM per poll, CDP `Runtime.evaluate(custom_js)` + hash-diff + Telegram-on-delta. Old `survival_message_check` cron paused; new watcher (120s, JS returns `{unread, rooms:[{r,t}]}`). Lesson — don't rewire crons to call inbox-check skills; the watcher framework was always the right answer. Same commit: `is_logged_in` Cloudflare false-positive fix (trusts already-on-upwork.com tab), `reply_mode` on `UpworkBotBehavior` (`notify_draft` = alert+draft; `auto` = sends, sensitive cats still escalate), `set_upwork_bot_behavior` syncs `inbox_check_cron`→`agent_jobs.cron_expression`, 2026 `room_<hex>` parser, multi-space contact-name dedup.
- **Live-browser watcher routing** (2026-05-16, 6aac606): Cloudflare-protected hosts MUST poll through the user's live signed-in Brave on `cdp_port` — fresh headless with copied profile fails the JS/TLS fingerprint challenge silently and the watcher hashes `{"unread":0,"rooms":[]}` forever. `_LIVE_BROWSER_WATCHER_HOSTS_BUILTIN = {upwork.com, linkedin.com}` + per-user `users.settings.browser.live_hosts` extras. `_check_watchers` routes per-watcher; `watcher.check_watcher(passive=True)` never goto-steals user's current tab and never focus-steals (`switch_tab(focus=False)` skips `Page.bringToFront`). Skills: `add_live_browser_host` / `remove_live_browser_host` / `list_live_browser_hosts`.
- **Contract intake pipeline** (2026-05-16, 6aac606): 5-PR feature: `survival_contract_intake` cron (every 6h) → `upwork_contract_poll` dedups via `users.settings.survival.seen_contract_urls` (FIFO 100) → fires `new_contract_intake(contract_url)` per new contract → worker LLM classifies work_type (6 buckets) + pre-extracts known facts → wraps `GoalExecutor.start()` with checklist hints → ONE batched Telegram ask via `answer_goal_questions`. On answer completion, `goal_executor._DEFAULT_DISPATCH_BY_SLUG['upwork']` resolves to `runtime/contract_intake_executor.dispatch_contract_intake` → 8-step provisioning (vault → live_host → account → watcher → template → setup-doc note → escalate-login → mark DONE). `execute_contract_intake_setup(goal_id)` manual retry for BLOCKED goals. Each work_type has its own checklist (web_monitoring=9, data_scraping=7, content_creation=5, code_dev=5, customer_support=5, other=1).
- **Telegram 1-tap-accept callback** (2026-05-16, 6aac606): contract-intake watchers fire `🔔` + InlineKeyboard `✅ Accept` / `⏭ Skip`. `push_telegram(inline_keyboard=...)` accepts 2D button matrix (callback_data clamped to 64 bytes). `build_watcher_context(accept_template_slug=...)` persists the slug in the encrypted watcher context. Daemon's watcher push branches on `accept_template_slug`: with-slug → keyboard push, without → legacy plain text. `telegram_commands._handle_callback` accept: branch sets template active (`active_templates.set_active`) for next browser turn.
- **Sonnet read-only-list dedup** (2026-05-16, 6aac606): hardening for `_dedup_tool_calls` in `claude_sdk_provider.py`. New `_READ_ONLY_LIST_SUFFIXES` collapses 22 listing tools (`upwork_get_messages`, `list_jobs`, `vault_list`, `survival_status`, `goal_status`, etc.) to FIRST occurrence regardless of args. Catches the "exploration loop" pattern observed 2026-05-16: 7× `upwork_get_messages` with varied limit/filter args wasted ~20s of Cloudflare round-trips before brain moved on. Two-pass dedup — listing collapse runs before exact (name, args) dedup.
- **`upwork_last_conversation` skill** (2026-05-16, 6aac606): zero-LLM deterministic fetch-latest-conversation (`upwork_get_messages(limit=1)` → `upwork_get_conversation`). `me_name` pulled from `SkillsProfile.display_name`. Instant_dispatch route catches "tell me what's in last conversation" + 11 other phrasings — bypasses brain LLM entirely. Replaces 2m33s brain loop with <10s skill call. The brain-routed path on the same intent caused the 2026-05-16 chunked-spam incident.
- **mcp-upwork Tiptap-safe typing** (2026-05-16, 6aac606): `send_message` now routes contenteditable typing through `_type_with_soft_breaks(text)` which splits on `\n` and uses `Shift+Enter` between lines (Tiptap's hard-break shortcut). Raw `keyboard.type(multi_line_text)` was emitting `\n` as literal Enter keystrokes → Tiptap binds Enter to Send → a 10-line draft fired 10 separate Upwork bubbles (2026-05-16 James Blue incident). All audited typing paths: send_message FIXED, cover-letter/rate/bid/answers fields use `.fill()` on textarea/input (newline-safe today; vulnerable if Upwork migrates to Tiptap).
- **mcp-upwork `draft_only` + `edit_message`** (2026-05-16, 6aac606): `SendMessageParams.draft_only=True` types into compose box but doesn't click Send (human-in-loop review path). New `EditMessageParams` + `edit_message` tool edits your own message within Upwork's ~60min edit window — same Tiptap-safe typing, `status='expired'` when window passed, URL + product-pitch guards fire BEFORE any browser nav. Both exposed via `upwork_send_message(draft_only=...)` and `upwork_edit_message(...)` server tools.
- **Tab reaper** (2026-05-16, 6aac606): `lazyclaw/browser/tab_reaper.py` + heartbeat `_check_tab_health()` every 5 ticks. `sweep_stale_tabs(idle_seconds=600)` closes unfocused tabs (active + anchored watcher tabs + system tabs always preserved). `enforce_tab_cap(max_tabs)` respects cap from heartbeat (not just on goto) — anchored watcher tabs preserved EVEN if total over cap. `refresh_white_screens()` runs the `_WHITE_SCREEN_JS` heuristic (text<10, children<3, interactive<1, readyState=complete) + reloads blanks; per-tick dedup prevents reload loops; `focus=False` so user's tab focus never stolen. Settings: `idle_tab_close_seconds=600`, `auto_refresh_white_screens=True` (defaults).
