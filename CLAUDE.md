...
Brain = thin router; work is delegated via `agent(agent_type, task, run_in_background)` to declarative specialists — `.md` + YAML frontmatter in `teams/specialists/`, loaded by `teams/specialist_loader.py`; custom ones live encrypted in DB. **The `tools:` allowlist in each specialist file is the callability contract** — `search_tools` discovery does NOT grant callability, and a prompt-named tool missing from the allowlist causes a stuck loop. When adding a capability to a specialist prompt, add the tool to its allowlist in the same change; audit by capability (reads AND writes) and ACROSS the roster (a capability no intuitive specialist carries is a routing dead-end). `startup_specialist_self_check` warns on drift at boot. Each specialist's `description:` frontmatter compiles into the dispatch schemas' `AGENT_TYPE_ROSTER` (CI-gated) — keep it current when a specialist's domain changes. N `agent` calls in one message run in parallel (`parallel_safe` batching); sync browser dispatches auto-promote to background (dispatched-ack now, consolidated report later).
