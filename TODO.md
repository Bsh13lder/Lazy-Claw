...
- [x] mcp-upwork shared-tab race (2026-08-20 22:00) — FIXED 08-21: every `@mcp.tool()` runs under a module-wide `@serialized` lock (navigate+settle+scrape atomic per call; CI-gated). Same commit: automation never steals screen focus — `switch_tab` defaults `focus=False`, `new_tab` defaults `background=True`, OAuth tab is the sole explicit foreground
...
- [x] Unify click/type paths through bezier+cadence (2026-08-15, cac3e2c) — 4 raw loops fixed
- [x] Tab-pinning from agent-browser (2026-08-15, 2ff98cf) — specialist tabs registered as owned + background + stale-pin recreate; foreground hijack closed
- [ ] Optional deeper hardening (not ported): Vercel's persisted {session}.target sidecar (daemon-restart survival) + browser-context isolation — only if long-lived interactive pins become a need
...
- [x] **Next-level pass (2026-06-12)** — sync safety + sheet hyperlinks + tags + power editing. **Sync**: optimistic concurrency (PUT `base_updated_at` → 409 w/ full current snapshot) across sheets/docs stores + name-optional save (kills stale-rename clobber); mobile conflict banner (Reload/Keep-mine) + fresh-on-focus; web 409 confirm via `reloadToken`. **Sheet hyperlinks**: `SHEET_HYPER_LINK_PLUGIN` resource helpers + URL/markdown `convert_urls_to_links` in `sheets/snapshot.py`; `POST /api/sheets/{id}/links/convert` + `convert_sheet_links` skill; web mounts `preset-sheets-hyper-link`; mobile renders/opens/inserts/auto+bulk-converts. **Web full free-preset surface**: filter, sort, conditional-formatting, data-validation, find-replace, note (+ hyper-link) — all Apache-2.0, NO Pro/charts/pivot. **Tags**: plaintext `tags` column on sheets/docs/pdf + chips/filter/editor on web + mobile (PDF via `PATCH /api/pdf/{id}`). **Mobile native grid power-editing**: range selection + drag handle, formatting toolbar (B/I/U/S, colors, align, wrap, number formats), row/col insert/delete/clear/width, TSV copy/paste, sort, freeze (visual; true pinning = freeze-v2 TODO), undo/redo (50). New mobile files: `univer_model.dart`, `univer_ops.dart`, `univer_links.dart`, `sheet_selection.dart`, `sheet_toolbar.dart`, `sheet_grid.dart`, `sheet_formula_bar.dart`, `sheet_link_ui.dart`, `sheet_conflict_banner.dart`, `tag_editor_sheet.dart`. (server 227 + mobile 1854 tests green; web tsc+build clean)
- [ ] **Deploy** — `docker compose build lazyclaw lazyclaw-web && docker compose up -d` (source baked into images). Optional: add LibreOffice to gateway image for doc→PDF export. Mobile APK via `scripts/build-mobile-apk.sh`.
- [ ] **Freeze v2** — true sticky-header pinning in the mobile grid (multi-viewport sharing a `TransformationController`); current freeze is visual-only (elevated bg + thicker border).
...
## Specialist-First Dispatch — Modes + Declarative Specialists (ADR-0005, in progress 2026-06-07)
Shift from a 277-tool generalist brain toward a thin-router + declarative specialists. See `docs/adr/0005-specialist-first-dispatch.md` + `docs/superpowers/plans/2026-06-07-specialist-first-dispatch.md`. Incremental + feature-flagged; no grounding defense removed before it migrates into a channel specialist.
- [x] **Phase 1 — Declarative loader**: `.md`+YAML format, `teams/specialist_loader.py` (reuses `lazybrain/frontmatter`), 3 builtins converted to `.md`, inline defs deleted, package-data wired. Byte-identical, zero behavior change.
- [x] **Phase 2 — Validation + CRUD**: `validate_specialist_tools` (fail-loud on unknown tools) + `/api/specialists` GET/POST/PUT/DELETE (builtins read-only) + web `Specialists.tsx` + mobile Specialists screen. Fixed latent `decrypt` import bug (custom specialists were silently failing to load).
- [x] **Phase 2.5 — Full specialist taxonomy (15 builtins, no duplicates)**: generated from the live registry (every `tools:` validated, zero unknowns). 5 original (browser/code/research + code_research/web_research) + 10 new domain specialists: `freelance` (survival/Upwork, +grounding), `email` (+grounding), `messaging` (WA/IG/TG, +grounding), `notes` (LazyBrain+memory), `tasks` (tasks+budgets), `documents` (sheets/docs/pdf), `contacts` (contacts+pipeline), `automation` (n8n+mcp+google), `bounty`, `system` (config/ops). `delegate` short-aliases auto-built from BUILTIN_SPECIALISTS (41 aliases). Channel specialists carry F1/grounding rules (= Phase 5a precondition).
- [x] **Phase 2.6 — Web nav consolidation**: the 3 skill-related Tools pages (Skill Hub · Skills · Specialists) folded into one `SkillsHub.tsx` with tabs **[Specialists (default) · Skills · Discover]**; Tools group now `["skills-hub", "mcp"]`. Old `?page=` deep links still resolve. `tsc -b` + `vite build` clean.
...
### Thin-Router Brain (2026-06-08)
Brain = always-free team lead: answers / does ≤1 quick action / else delegates; workers run to completion and report back; escalation = a worker returns a `{blocked}`-style result and the brain decides (Claude Code model, NOT a mid-run pause); autonomy governed by the 4 modes. Spec: `docs/superpowers/specs/2026-06-08-thin-router-brain-design.md`; plan: `docs/superpowers/plans/2026-06-08-thin-router-brain.md`.
- [x] **Phase 1 — `delegate` = dispatch-and-exit** (ON MAIN, `aa320c0`): `agent.py` recognizes `delegate` as fire-and-forget across the 3 foreground guards (AUTO-PROMOTE exclusion + both action-claim guards) + hard-stop exiting the turn after a successful `delegate`. Fixes the 14:18 triple-execution bug (delegate → didn't exit → action-claim retry ran the work → AUTO-PROMOTE spawned a 3rd executor; specialist result orphaned).
- [x] **Phase 2 — 1-action routing cap** (ON MAIN, in `aa320c0`), behind `LAZYCLAW_THIN_ROUTER` (default OFF): `_META_TOOLS` set (delegate/dispatch_subagents/run_background/search_tools/web_search/recall_memories/save_memory/get_agent_status); after one inline NON-meta call the brain narrows to meta-only so it MUST delegate; AUTO-PROMOTE skipped under the flag. Default off = zero behavior change.
- [x] **Specialist MCP allowlist + Upwork submit** (ON MAIN, `e83fdc2`): `teams/runner.py` `_filter_tools` + per-call gate match MCP tools by BARE suffix (`_bare_tool_name`, strips `mcp_<uuid>_`) against `allowed_skills`. `freelance_specialist.md` gains `upwork_submit_proposal` + "submit via upwork_submit_proposal, never re-call apply_job". Restores proposal SUBMISSION (was stuck in an apply_job draft-loop); same fix unblocks messaging/email specialists reaching whatsapp_read/email_read.
- [x] **Phase 3a — mode relabel** (COMMITTED `5baab1d`): canonical UI names **Ask / Plan / Action / Execute** (display+input only; enum values stay `chat/ask/plan/auto`, NO migration). `MODE_LABELS` (CHAT→"Ask", ASK→"Action", PLAN→"Plan", AUTO→"Execute") + new `parse_mode_label()`; Telegram `/act` uses it. Map: Ask=no-tools (chat), Plan=read-only+gate (plan), Action=default (ask), Execute=autonomous (auto).
- [x] **Phase 3b — web/mobile labels** (COMMITTED `cf3ea24`): `web/src/components/ModeSwitch.tsx` + `web/src/api.ts` comment + mobile `settings_repository.dart` (`kAgentModeLabels`) + `settings_screen.dart` help caption — labels reordered Ask/Plan/Action/Execute, values unchanged. `tsc -b` + `flutter analyze` clean.
- [x] **Phase 3c — search-loop cap** (COMMITTED `cf3ea24`): `llm/providers/claude_sdk_provider.py` `_dedup_tool_calls` caps tool-discovery (`_DISCOVERY_TOOL_NAMES = {search_tools}`, `_DISCOVERY_TOOLS_CAP = 3`) to first 3 per assistant turn. Single point → fixes brain AND worker. Kills the panic-search loop (a documents_specialist emitted ~19 search_tools in one turn).
- [x] **Phase 4a — mechanical F1 into specialists** (COMMITTED `a6ff1ed`): new `runtime/f1_gate.py` (`evaluate_grounding(reply, tool_history, tool_results) -> GroundingVerdict`) composes EXISTING detectors (`detect_confabulation` + `phase2_enforcement_verdict` + `build_raw_data_injection`), fails OPEN on error, gates only channel-read turns. Wired into `teams/runner.py` reply path with a bounded corrective re-roll (max 2) → `[grounding-degraded]` on exhaustion. Brain F1 in `agent.py` UNTOUCHED. Closes the gap where channel-reading specialists had ZERO mechanical anti-confab. 185 F1-suite tests stay green + 12 new gate tests.
- [x] **Phase 4b — cap chat-responsiveness** (COMMITTED `a6ff1ed`): `agent.py` cap now sets `_thin_router_capped`/`_cap_iter`; a cap-aware failsafe auto-submits the user message to a background task if the brain refuses to delegate under the cap (mirrors the AUTO-PROMOTE "narrowed-but-refused" failsafe, which gated on `_force_dispatch_only` the cap never set). Makes `LAZYCLAW_THIN_ROUTER` safe to flip on.
- **Soak status (2026-06-10)**: `LAZYCLAW_THIN_ROUTER=1` is ON in prod `.env`. First soak finding (2026-06-10 00:33): brain delegated `upwork_get_conversation` to the freelance specialist whose allowlist lacked the raw upwork read tools its own prompt promises — worker stuck-looped on `search_tools` and died. Fixed on `fix/dispatch-audit-hardening` (allowlist + regression test + startup self-check). Containment worked as designed.
- [ ] **Phase 4c — brain teardown** (DEFERRED, pending production soak): delete ONLY the AUTO-PROMOTE *trigger* + the inline *pivot detector* (the routing machinery the cap makes redundant). **KEEP-LIST (do NOT delete):** SDK dedup + discovery cap (provider hygiene), ALL keyword-gating (tool selection), the stuck-detector (meta-tool/browser loops the cap can't catch), the 4 AUTO-PROMOTE failsafes (chat-responsiveness). **Precondition:** flip `LAZYCLAW_THIN_ROUTER=1` and SOAK in prod first. The spec's original "delete dedup + keyword-gating" was DROPPED after research showed it reopens old bugs.
