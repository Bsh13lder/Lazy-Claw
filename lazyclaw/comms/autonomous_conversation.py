...
Scope — this module drives OUTBOUND conversations the agent starts on the
user's behalf ("ask Alice on WhatsApp if she's coming"): there is no prior
thread, so the opener is drafted from the GOAL and gated behind a first-message
approval before anything is sent.

Not to be confused with :mod:`lazyclaw.comms.conversation_runner`, which
handles the inbox "Ask AI" button on an EXISTING thread. That path must read
the live thread FIRST and compose from what was actually written (the grounding
contract), so it stays a single grounded agent turn with no approval gate.
Two different jobs, two different mechanisms — keep them apart.
"""
...
async def _read_new_contact_messages(config: Config, deps: RunnerDeps, conv: dict) -> list[dict]:
    ...
    ``read_thread`` returns a typed :class:`ReadResult`: a FAILED read
    (``ok=False`` — MCP down, channel not configured) must NOT be mistaken for
    "the contact hasn't replied yet", or the runner would back off and then
    give up with "no clear answer" while the channel was simply dead. On a
    failed read we return no messages, which backs off and retries — the
    conversation stays alive instead of silently failing.
    """
    gw = build_gateway(deps.registry, conv["user_id"])
    res = await gw.read_thread(conv["channel"], conv["contact_handle"])
    if not res.ok:
        logger.warning(
            "conversation %s: channel read failed (%s) — backing off",
            conv["id"], res.error,
        )
        return []
    seen = {t["text"] for t in conv["transcript"]}
    return [{"sender": m.sender, "text": m.text, "ts": m.timestamp}
            for m in res.messages if not m.is_mine and m.text and m.text not in seen]
