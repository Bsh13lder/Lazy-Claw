...
- **Goal Executor**: `runtime/goal_executor.py` — autonomous high-level objectives. States: `DRAFTING → AWAITING_USER_INFO → EXECUTING → DONE/BLOCKED/FAILED/ABORTED`. Encrypted `goals` table. Key UX: questions surface in ONE batch upfront. 6 skills.
...
- **`upwork_last_conversation` skill + instant_dispatch**: zero-LLM deterministic fetch — bypasses brain. Replaces 2m33s brain loop with <10s skill call.
