...
def register_crawl_tools(mcp, get_modules):
    ...
    async def crawl_url(
        ...
        """Fetch a single URL and return clean markdown of the page content. JS-rendered. Use this when web_search returned a URL but the snippet didn't carry the field you need. Cheaper than browser. Set wait_for_js=true for SPAs. For very long pages, set output_path=/tmp/page.md to persist the full content and get only metadata back."""
        # Input validation
    ...
    async def deep_crawl_site(
        ...
        """Crawl multiple linked pages from one site (BFS/DFS) up to max_depth and max_pages. Use this for site mapping or "scrape all product pages on this domain" — NOT for one-off page reads (use crawl_url for that). Set output_path=/tmp/site/ to persist per-URL markdown + index.json so the response stays small."""
